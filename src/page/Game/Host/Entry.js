...
class Entry extends Component {
  ...
  render() {
    ...
    if (this.state.begin) {
      let { from } = this.props.location.state || { from: { pathname: "/host/ready" } };
    ...
    return (
      <div>
        <div>
          ...
          <div><h1 style={{ fontSize: '2em' }}>tham gia bằng mã PIN:</h1></div>
          <div id="inner" style={{ marginTop: '10px', width: '300px', height: '90px', background: '#fff', color: '#000' }}>
            <h1 className='id_game'>
              <b>
                {this.props.game ? this.props.game.code : ''}
              </b>
            </h1>
          </div>
          ...
          <div style={{ margin: '35px 0', fontSize: '8px' }}>
            <div className='row' style={{ fontSize: '2em' }}>
              ...
              <div className='col-4 name' >{game.info ? game.info.title : 'BAMBOO'}</div>
              <div className='col-4 '>
                <button className='start' style={{ marginTop: '20px', width: '30%', height: '50px' }} onClick={this.begin}>
                  BẮT ĐẦU
                </button>
              </div>
            </div>
