...
class Info extends Component {
  ...
  render() {
    ...
    const { from } = this.props.location.state || { from: { pathname: "/host" } };
          ...
          <div style={{ padding: '0 1em' }}>
            <div className='row'>
              <div className='show-detail-quiz col-12 col-sm-4 col-md-4'>
                <img src={info ? info.img ? info.img : '/images/img_quest_default.jpg' : '/images/img_quest_default.jpg'} style={{ width: '100%', height: '15em', objectFit: 'contain', marginBottom: '10px' }} />
                <h4 style={{ margin: '10px 0', whiteSpace: 'pre-wrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {info ? info.title : 'Tên thử thách'}
                </h4>
                <p style={{ margin: '10px 0' }}>{info ? info.description : 'Miêu tả thử thách'}</p>
                <Link to='#' onClick={this.startGame} className='btn btn-success' style={{ width: '100%', margin: '10px 0' }}>Tạo trò chơi</Link>
              </div>
              <div className='col-12 col-sm-8 col-md-8' style={{ marginBottom: '30px' }}>
                <h3 style={{ marginBottom: '10px', textAlign: 'left' }}>Các câu hỏi của thử thách</h3>
                ...
                <div className='row' >
                  ...
                  {
                    isOwnner ?
                      <div className='col-12 col-sm-12 col-md-12' style={{ textAlign: 'center' }}><Link to={'/quest/add/' + info._id}><i className="fas fa-plus-circle" style={{ fontSize: '50px', color: 'blue' }} /></Link></div> : null
                  }
